...
import { useDeliveryTicketProductTicketList } from '@/composables/delivery/ticketProductOrderManagement/useDeliveryTicketProductTicketList'
...
deliveryOrderStatusTicketListOptions
...
} = useDeliveryTicketProductTicketList({ onShowModalCancelRequest })
</script>
...
<template>
  <div>
        ...
        <div class="wf_flex wf_items-center">
          <FormGroup class="wf_flex-1" custom-class="wf-pb-1 wf-pl-11" label="주문상태">
            <WelfareSelect
              ...
              placeholder="전체"
              :disabled="!!values.keywordSearchType"
              :options="deliveryOrderStatusTicketListOptions"
    ...
    <div class="wf-p-30">
      ...
      >
        <template #columns>
          <Column key="purchaseConfirm" columnKey="purchaseConfirm" header="구매확정처리" class="wf_m-w-200">
            <template #body="slotProps">
              <WelfareMdButton
                label="구매확정"
                button-size="small"
                button-type="default"
                :disabled="isLoadingPurchaseConfirm(slotProps.index, slotProps.data.id)"
                @on-click="() => handleConfirmPurchase(slotProps.data.receiverProductOrderKey, slotProps.data.issueDate, slotProps.index)"
              />
            </template>
          </Column>
          <Column
          ...
          >
            <template #body="slotProps">
              <span v-if="!item?.onClick">{{ slotProps.data[item.field] }}</span>
        ...
        <template #buttons>
          ...
          <WelfareMdButton label="구매확정" class="wf_w-68" buttonType="default" @on-click="() => handleConfirmPurchaseMultiple()" />
...
<script lang="ts">
export default {
  name: 'DeliveryTicketProductTicketListPage'
}
</script>
