...
import { FORMAT_DATE_YYYY_MM_DD_HH_mm_ss, formatTime, renderLabelEnum } from '@/common'
import {
WelfareBigButton,
WelfareCheckbox,
WelfareColorButton,
...
import { useDeliveryCancelRequestModal } from '@/composables/delivery/modal/useDeliveryCancelRequestModal'
import { useDeliveryCancelSellModal } from '@/composables/delivery/modal/useDeliveryCancelSellModal'
import { useDeliveryIssuanceDelayInfoModal } from '@/composables/delivery/modal/useDeliveryIssuanceDelayInfoModal'
import { useDeliveryTicketProductOrderDetail } from '@/composables/delivery/ticketProductOrderManagement/useDeliveryTicketProductOrderDetail'
...
import { deliveryOrderStatusOrderDetailOptions } from '@/models/views/delivery/ticketProductOrderManagement/DeliveryTicketProductOrderDetailModel'
import {
...
deliveryClaimStatusOrderDetailOptions,
...
const { onShowModal: onShowModalCancelRequest } = useDeliveryCancelRequestModal()
const { openModal: openModalCancelSell } = useDeliveryCancelSellModal()
const { onShowModal: onShowModalCancelRequestIssuanceDelay } = useDeliveryIssuanceDelayInfoModal()
...
} = useDeliveryTicketProductOrderDetail({ onShowModalCancelRequest, openModalCancelSell, onShowModalCancelRequestIssuanceDelay })
</script>
  ...
  <div>
    <div class="wf-p-30 wf-space-y-29">
        ...
        <div class="wf_flex wf_items-center">
          <FormGroup class="wf_flex-1 wf-form-group-small" custom-class="wf-pb-1 wf-pl-11" label="회원">
            <MemberSearchWithTableWrapperModal
              ...
              label-button="조회"
              :model-value="values.orderMemberKey"
              :disabled="!!values.keywordSearchType"
              is-disabled-on-empty-search-text
              @on-open-popup-search-member="handleOpenPopupSearchOrderMember"
            />
          </FormGroup>
        ...
        <div class="wf_flex wf_items-center">
          <FormGroup class="wf_flex-1" custom-class="wf-pb-1 wf-pl-11" label="주문상태">
            <WelfareSelect
              ...
              placeholder="전체"
              :disabled="!!values.keywordSearchType"
              :options="deliveryOrderStatusOrderDetailOptions"
          ...
          <FormGroup class="wf_flex-1" custom-class="wf-pb-1 wf-pl-11" label="클레임 상태" is-border-left>
            <WelfareSelect
              ...
              placeholder="전체"
              :disabled="!!values.keywordSearchType"
              :options="deliveryClaimStatusOrderDetailOptions"
      ...
      <div class="wf_flex wf_justify-center wf-gap-x-20">
        ...
        <WelfareBigButton label="조회" button-type="default" @on-click="() => onSubmitAndReset()" />
      </div>
    </div>
    ...
    <div class="wf-p-30">
      ...
      >
        <template #columns>
          <Column key="ticketIssuance" columnKey="ticketIssuance" header="티켓발급" class="wf_m-w-200">
            <template #body="slotProps">
              <WelfareColorButton
                ...
                button-type="red"
                :disabled="
                  isLoadingTicketIssue(slotProps.index, slotProps.data.id) || slotProps.data.claimStatus === DeliveryOrderStatusModel.SALE_CANCEL
                "
                @on-click="() => handleOpenConfirmTicketIssue(slotProps.index, slotProps.data.receiverProductOrderKey)"
              />
            </template>
          </Column>
          <Column
          ...
          >
            <template #body="slotProps">
              <span v-if="item.field === 'paymentDate'">{{ formatTime(slotProps.data.paymentDatetime, FORMAT_DATE_YYYY_MM_DD_HH_mm_ss) }}</span>
              <span v-if="item.field === 'md'">{{ `(${slotProps.data.mdKey}) ${slotProps.data.mdName}` }}</span>
              <span
                v-if="item?.onClick && (!item.compareValue || (item?.compareValue && slotProps.data[item.field] === item?.compareValue))"
                class="wf-underline wf-pointer wf_text-sub-02"
                @click="() => item.onClick && item.onClick(slotProps.data)"
                >{{ item?.options ? renderLabelEnum(item?.options ?? [], slotProps.data[item.field]) : slotProps.data[item.field] }}</span
              >
              <span v-else>{{ item?.options ? renderLabelEnum(item?.options ?? [], slotProps.data[item.field]) : slotProps.data[item.field] }}</span>
            </template>
          </Column>
        </template>
        <template #buttons>
          <WelfareMdButton label="취소신청" class="wf_w-69" buttonType="liner" @on-click="() => showModalCancelRequest()" />
          <WelfareMdButton label="판매취소" class="wf_w-69" buttonType="liner" @on-click="() => showModalCancelSell()" />
          <WelfareMdButton label="발급지연안내" class="wf_w-93" buttonType="liner" @on-click="() => showModalCancelRequestIssuanceDelay()" />
          <WelfareColorButton label="티켓발급" class="wf_w-68" buttonType="red" @on-click="() => handleOpenConfirmTicketIssue()" />
...
<script lang="ts">
export default {
  name: 'DeliveryTicketProductOrderDetailPage'
}
</script>
