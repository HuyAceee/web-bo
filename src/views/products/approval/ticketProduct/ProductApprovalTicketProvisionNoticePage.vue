...
import {
  ProductTicketDetailBasicInfoProps,
...
const {
  ...
  {
    data: { productRequestKey: Number(route.query.productRequestKey) },
    category: props.category
  },
  { type: ProductTicketNoticeProvidingType.EditApproval }
)

const checkLastProductProgressStatusCode = computed(() => {
  return (
    !currentProvisionData?.value?.lastProductProgressStatusCode ||
    currentProvisionData?.value?.lastProductProgressStatusCode === PRODUCT_STATUS_ENUM.PRODUCT_REGISTRATION_IN_PROGRESS
  )
})
</script>
...
<template>
  <div class="">
    ...
    />
    <ProductBottomButton
      v-if="checkLastProductProgressStatusCode"
    ...
    <ProductBottomButton
      v-else
      @on-cancel-click="openConfirmCancel"
      @on-approval-finished-click="handleApprovalById"
      @on-approval-reject-click="handleRefuseApprovalById"
      @on-save-click="onSaveClick"
      previewDisabled
      temporaryDisabled
      save-disabled
