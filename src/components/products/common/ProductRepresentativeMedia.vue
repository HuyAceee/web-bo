...
import { DataTableContainer, WelfareInputText, WelfareMdButton, WelfareRadio } from '@/components/uikit'
        ...
        <div class="wf_flex wf-space-x-20">
          <div class="wf-body-popup--item wf_flex wf_flex-1 wf_flex-col wf-space-y-12">
            ...
            <div class="wf_re-product-table-create">
              <DataTableContainer :value="dataTable" hidden-pagination is-select-invisible :column-configs="props.configHeader ?? []">
                <template #header-imageRepresentativeYn="{ props }"> {{ props }}<span class="wf_text-sub-01">*</span> </template>
                <template #body-id="{ props: data }">
                  <div class="wf_flex wf_items-center">
                    <span class="wf-body_03-reg wf-pt-1">{{ data.index + 1 }}</span>
                  </div>
                </template>
                <template #body-imageRepresentativeYn="{ props: data }">
                  <WelfareRadio
                ...
                <template #body-name="{ props }">
                  <span class="wf-body_03-reg wf_flex-1 wf_justify-start wf-pb-2 wf-pl-23 wf_underline">{{ props.data.originName }}</span>
                </template>
                <template #body-action="{ props }">
                  <div class="wf-space-x-4 wf_flex wf-pt-1">
                    <WelfareMdButton
                      label="등록"
                      buttonType="liner"
                      :button-size="'small'"
                      @on-click="onRegisterFileModal(props.data.id as number)"
                    />
                    <WelfareMdButton
                      label="삭제"
                      buttonType="liner"
                      :button-size="'small'"
                      :disabled="disabledDeleteBtn(props.index, props.data.url)"
                      @on-click="$emit('deleteFile', props.data)"
                    />
                  </div>
                </template>
              </DataTableContainer>
          ...
          <div class="wf-body-popup--item wf_flex wf_flex-1 wf_flex-col wf-space-y-12">
            ...
            >
              <img
                ...
                alt=""
                :src="dataTable[imageChecked]?.urlReview"
              />
