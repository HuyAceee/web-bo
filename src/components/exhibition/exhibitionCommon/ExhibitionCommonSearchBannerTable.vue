...
import { useExhibitionCommonSearchBannerTable } from '@/composables/exhibition/exhibitionCommon/useExhibitionCommonSearchBannerTable'
import {
...
const props = defineProps<ExhibitionCommonSearchBannerTableProps>()
const emit = defineEmits<ExhibitionCommonSearchBannerTableEmits>()
...
const {
  ...
  onUpdate,
  onExport
} = useExhibitionCommonSearchBannerTable(props, emit)
...
<template>
  <div class="wf-mx--1">
    ...
    >
      ...
      <template #columns>
        <Column v-for="(col, index) in exhibitionCommonSearchBannerTableConfig" :key="index" :class="col?.class" :field="col?.field">
          <template #header>
          ...
          <template #body="slotProps">
            ...
            <template v-else-if="col.field == 'searchWordBannerKey'">
              <span class="wf-text_link wf-pointer" @click="() => onUpdate(slotProps.data)">{{ slotProps.data?.searchWordBannerKey }}</span>
            </template>

            <template v-else-if="col.field == 'searchWordBannerKeyword'">
              <div class="wf_w-full">
                <WelfareInputText
                  size="small"
                  placeholder=" "
                  :model-value="values?.data[slotProps.index]?.searchWordBannerKeyword"
                  @keypress="acceptMaxKeyWord"
                  @update:model-value="(val) => onChangeMaxKeyword(`data.${slotProps.index}.searchWordBannerKeyword`, val)"
                />
              </div>
            </template>
