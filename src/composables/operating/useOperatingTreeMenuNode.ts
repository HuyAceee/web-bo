...
export const useOperatingTreeMenuNode = (emits: OperatingTreeMenuNodeEmits, tree: Ref<TreeNode[]>, props?: OperatingTreeMenuNodeProps) => {
  ...
  const contextMenuActions = operatingMenuActions
  const contextMenuLeafActions = operatingMenuActions.slice(1, operatingMenuActions.length)
  ...
  const handleActionMenuClick = (action: OperatingMenuPopupAction) => {
    showMenu.value = false
  ...
  onMounted(() => {
    ...
    if (props?.expandedMode === operatingExpanderActionsEnum.ALL) {
      expandAll()
    }
  })
