...
import { ExhibitionCommonSearchKeywordGetListParamsModelRequest } from '@/models/services/requests/exhibition/ExhibitionSearchKeywordModelRequest'
...
import {
  ExhibitionSearchKeywordMngtFormQueryModel,
...
export const useExhibitionCommonSearchKeywordMngt = () => {
  ...
  const applyChannelOptions: GenericOptionType<keyof ExhibitionSearchKeywordMngtFormQueryModel>[] = [
    {
      label: '전체',
  ...
  const defaultValue: ExhibitionSearchKeywordMngtFormQueryModel = {
    displayYn: 'all',
  ...
  const {
    ...
    clearData,
    validate
  } = useFormFilterTable<ExhibitionSearchKeywordModel, ExhibitionSearchKeywordMngtFormQueryModel>({
  ...
  const fetchData = async (page: number, size: number): Promise<DataTablePaginationResponseModel<ExhibitionSearchKeywordModel>> => {
    try {
      const query: ExhibitionCommonSearchKeywordGetListParamsModelRequest = {
        ...values,
      ...
      const { data, totalItems } = await exhibitionSearchKeywordApi.getList(query)
