...
import { useExhibitionCommonSearchKeywordPopup } from '@/composables/exhibition/exhibitionCommon/useExhibitionCommonSearchKeywordPopup'
...
import {
  ExhibitionCommonSearchKeywordDeleteDataModelRequest,
  ExhibitionCommonSearchKeywordPutDataModelRequest
} from '@/models/services/requests/exhibition/ExhibitionSearchKeywordModelRequest'
import {
  ExhibitionCommonSearchKeywordTableEmits,
  ExhibitionCommonSearchKeywordTableFormModel,
  ExhibitionCommonSearchKeywordTableProps,
  ExhibitionSearchKeywordModel,
...
export const useExhibitionCommonSearchKeywordTable = (props: ExhibitionCommonSearchKeywordTableProps, emit: ExhibitionCommonSearchKeywordTableEmits) => {
  ...
  const { openModal: openSearchKeywordPopup } = useExhibitionCommonSearchKeywordPopup()
  const tableRef = ref()
  ...
  const schema = {
    data: array().of(
      object().shape({
        isChanged: boolean(),
        searchBoxKeywordName: string().trim().nullable().when(['isChanged'], {
  ...
  const { values, setFieldValue, validate } = useForm<ExhibitionCommonSearchKeywordTableFormModel>({
  ...
  const tableComputed = computed(() =>
    (values.data ?? []).map((v) => ({
      ...v,
      id: v.searchBoxKeywordKey,
      isSelected: checkedList.value.includes(v.searchBoxKeywordKey as number)
    }))
  )

  const onCheckAllChange = (value: boolean) => {
    if (value) {
      checkedList.value = values.data.map((it) => it.searchBoxKeywordKey as number)
    } else {
      checkedList.value = []
    }
  }
  ...
  const onSave = async () => {
    ...
    if (valid) {
      const reqData: ExhibitionCommonSearchKeywordPutDataModelRequest = {
  ...
  const handleSave = async (reqData: ExhibitionCommonSearchKeywordPutDataModelRequest) => {
    setLoading?.(true)
  ...
  const handleDelete = async () => {
    const reqData: ExhibitionCommonSearchKeywordDeleteDataModelRequest = {
  ...
  const onRegis = () => {
    openSearchKeywordPopup(() => emit('refresh'))
  }

  const onUpdate = (data: any) => {
    openSearchKeywordPopup(() => emit('refresh'), data, { type: 'modify' })
  }

  const onExport = () => {
    const recordHeader = exhibitionCommonSearchKeywordTableConfig.reduce((acc, cur) => {
      if (cur.field === 'url') {
        return [...acc, cur.header, '', '']
      }
      return [...acc, cur.header]
    }, [] as string[])
    const mergedHeaderConfig = exhibitionCommonSearchKeywordTableConfig
      .map((it, idx) => it.field === 'url' ? ({ s: { c: idx, r: 0 }, e: { c: idx + 2, r: 0 } }) : undefined)
      .filter(it => it !== undefined)
    const renderLinkData = (item: ExhibitionSearchKeywordModel) => {
    ...
    const recordData = tableComputed.value.map((it) => ({
      no: it.rowNum,
      searchBoxKeywordKey: it.searchBoxKeywordKey,
      applyChannelName: it.applyChannelName,
      searchBoxKeywordName: it.searchBoxKeywordName,
    ...
    exportExcel(recordData, "SearchKeyword", recordHeader, 12, {
